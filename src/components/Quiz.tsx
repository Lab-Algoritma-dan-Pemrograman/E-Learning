  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-3xl p-8 shadow-sm space-y-8">
      <div className="space-y-2">
        <div className="text-xs font-black text-emerald-600 uppercase tracking-widest">Kuis Singkat</div>
        ...
        {options.map((option, idx) => (
          <button
            ...
            className={cn(
              ...
              isSubmitted && idx === correctAnswer && "border-emerald-500 bg-emerald-50",
              isSubmitted && selected === idx && idx !== correctAnswer && "border-red-500 bg-red-50"
            )}
          >
            <span className={cn(
              "font-medium",
              selected === idx ? "text-emerald-900" : "text-zinc-600"
            )}>
              {option}
            </span>
            
            {isSubmitted && idx === correctAnswer && <CheckCircle2 className="text-emerald-500" size={20} />}
