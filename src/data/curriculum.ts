      ...
      },
      {
        ...
        "lessons": [
          ...
          },
          {
            ...
            "quiz": {"question":"Mengapa penentu format %[^\\n] lebih tepat daripada %s untuk membaca nama lengkap?","options":["Karena %[^\\n] membaca seluruh baris termasuk spasi, sedangkan %s berhenti di spasi","Karena %[^\\n] hanya bisa dipakai untuk membaca angka","Karena %[^\\n] tidak memerlukan operator alamat &","Karena %[^\\n] otomatis mengubah input menjadi huruf kapital"],"correctAnswer":0},
            "testCases": [{"expectedOutput":"Masukkan nama kota: Kota tujuan: Kuala Lumpur\n","description":"<span>Buat program yang meminta pengguna memasukkan nama kota (bisa mengandung spasi) menggunakan <code class=\"bg-zinc-100 px-1.5 py-0.5 rounded text-rose-700 font-mono text-xs\">scanf()</code> dengan penentu format <code class=\"bg-zinc-100 px-1.5 py-0.5 rounded text-rose-700 font-mono text-xs\">%[^\\n]</code>, lalu tampilkan kembali.</span>","input":"Kuala Lumpur"}],
            "validationRules": [{"message":"Gunakan: scanf(\"%[^\\n]\", kota);","pattern":"scanf\\s*\\(\\s*\"%\\[\\^\\\\n\\]\"\\s*,\\s*kota\\s*\\)\\s*;","shouldExist":true},{"message":"Buang baris baru dengan: getchar();","pattern":"getchar\\s*\\(\\s*\\)\\s*;","shouldExist":true},{"message":"Gunakan format specifier %s: printf(\"Kota tujuan: %s\\n\", kota);","pattern":"printf\\s*\\(\\s*\"Kota tujuan:\\s*%s\\\\n\"\\s*,\\s*kota\\s*\\)","shouldExist":true}]
          },
          {
            "id": "c-level-1-m4-l4",
